  ...
  return (
    <>
      <aside className={`
        ...
        bg-white/95 dark:bg-gray-900/95 lg:bg-white/90 lg:dark:bg-gray-900/90 
        ...
        shadow-2xl lg:shadow-xl shadow-gray-900/10 dark:shadow-black/20
        ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Enhanced Header with Close Button for Mobile */}
        <div className="relative bg-gradient-to-r from-gray-100 via-white to-gray-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 border-b border-gray-200/50 dark:border-gray-700/50">
          <div className="absolute inset-0 bg-gradient-to-r from-postman-orange/5 via-transparent to-postman-orange/5"></div>
          
          {/* Mobile Close Button */}
          <button 
            className="absolute top-3 right-3 lg:hidden p-2 rounded-xl text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200/50 dark:hover:bg-gray-700/50 transition-all duration-200 z-10"
            onClick={onClose}
          ...
          <div className="relative flex bg-white/10 dark:bg-gray-900/30 backdrop-blur-sm rounded-xl p-1">
            <SidebarTabButton
        ...
        {/* Footer with Stats */}
        <div className="border-t border-gray-200/50 dark:border-gray-700/50 bg-gray-100/50 dark:bg-gray-900/50 p-4">
    ...
    >
      ...
      <div className="flex flex-col items-center space-y-1.5">
        ...
        <span className={`text-[10px] lg:text-xs uppercase tracking-wider font-bold transition-colors duration-300 ${
          isActive ? 'text-gray-900 dark:text-gray-300' : 'text-gray-600 dark:text-gray-500 group-hover:text-gray-800 dark:group-hover:text-gray-300'
        }`}>
          {label}
        </span>
      </div>
  ...
  return (
    <div className="space-y-4 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex items-center justify-between">
        <h3 className="text-gray-900 dark:text-white text-sm font-bold tracking-wide">{t('history.title')}</h3>
        {history.length > 0 && (
          <span className="text-xs text-gray-600 dark:text-gray-400 bg-gray-200/50 dark:bg-gray-800/50 px-2 py-1 rounded-full">
      ...
      {history.length === 0 ? (
        <div className="text-center py-12">
          <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-gray-300 to-gray-400 dark:from-gray-700 dark:to-gray-800 rounded-2xl flex items-center justify-center shadow-lg">
            <svg className="w-8 h-8 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">{emptyMessage}</p>
          <p className="text-gray-500 dark:text-gray-500 text-xs mt-2">Make your first request to see it here</p>
        </div>
      ) : (
        <div className="space-y-3">
          {history.slice(0, 10).map((item, index) => (
            <div 
              key={item.id} 
              className="group relative flex items-center space-x-3 p-3 rounded-xl hover:bg-gradient-to-r hover:from-gray-200/50 hover:to-gray-100/30 dark:hover:from-gray-800/50 dark:hover:to-gray-700/30 cursor-pointer transition-all duration-300 border border-transparent hover:border-gray-300/30 dark:hover:border-gray-600/30 hover:shadow-lg hover:shadow-gray-900/10 dark:hover:shadow-black/20 transform hover:scale-[1.02]"
            ...
            >
              ...
              <div className="flex-1 min-w-0 relative z-10">
                <p className="text-sm text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white truncate transition-colors duration-300 font-medium">
                  {item.url}
                </p>
              ...
              {/* Arrow indicator */}
              <svg className="w-4 h-4 text-gray-400 dark:text-gray-600 group-hover:text-gray-600 dark:group-hover:text-gray-400 transition-all duration-300 transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const SavedContent: React.FC<{
  ...
  return (
    <div className="space-y-4 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex items-center justify-between">
        <h3 className="text-gray-900 dark:text-white text-sm font-bold tracking-wide">{t('saved.title')}</h3>
      ...
      {savedRequests.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">{emptyMessage}</p>
          <p className="text-gray-500 dark:text-gray-500 text-xs mt-2">Save your favorite requests for quick access</p>
        </div>
      ) : (
        <div className="space-y-3">
          {savedRequests.map((request, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0 relative z-10">
                <p className="text-sm font-bold text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white transition-colors duration-300 truncate">
                  {request.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-400 transition-colors duration-300 truncate">
                  {request.url}
                </p>
              </div>
...
const CollectionsContent: React.FC = () => {
  ...
  return (
    <div className="space-y-4 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex items-center justify-between">
        <h3 className="text-gray-900 dark:text-white text-sm font-bold tracking-wide">{t('tabs.collections')}</h3>
        <span className="text-xs text-gray-600 dark:text-gray-400 bg-gray-200/50 dark:bg-gray-800/50 px-2 py-1 rounded-full">
          Beta
        </span>
      </div>
      ...
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Collections coming soon</p>
        <p className="text-gray-500 dark:text-gray-500 text-xs mt-2">Organize your requests into collections</p>
        
        <div className="mt-6 p-4 bg-gradient-to-r from-purple-200/20 to-blue-200/20 dark:from-purple-900/20 dark:to-blue-900/20 rounded-xl border border-purple-300/10 dark:border-purple-500/10">
          <div className="flex items-center justify-center space-x-2 mb-2">
            <div className="w-2 h-2 bg-purple-500 rounded-full animate-pulse"></div>
            <span className="text-xs text-purple-600 dark:text-purple-300 font-medium">Coming in v2.0</span>
          </div>
